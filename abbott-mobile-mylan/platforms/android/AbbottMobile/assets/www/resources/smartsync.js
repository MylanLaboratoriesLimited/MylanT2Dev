(function($, _, Backbone, forcetk) {
        ...
        save: function(record, noMerge, isLocal) {
            ...
            return mergeIfRequested()
                .then(function(record) {
                    ...
                    if (isLocal) return smartstoreClient.upsertSoupEntriesWithExternalId(that.soupName, [ record ], that.keyField)
                    else return smartstoreClient.upsertSoupEntries(that.soupName, [ record ])
                })
                ...
                var mergeIfRequested = function() {
                    if (noMerge) {
                        ...
                        var smartSql = "SELECT {" + that.soupName + ":_soup} "
                            + "FROM {" + that.soupName + "} "
                            + "WHERE {" + that.soupName + ":__local__} = 'true' "
                        ...
                        return smartstoreClient.runSmartQuery(querySpec)
                            ...
                            .then(function() {
                                var recordWithoutCompoundProperties = function(record) {
                                    ...
                                    var propertiesToIgnoreOnClient = _.filter(keys, function(key){return _(key).contains('.')});
        ...
        find: function(querySpec, ignoreDeleted) {
            ...
            var buildQueryResponse = function(cursor) {
                var filterCursorEntries = function(cursor, ignoreDeleted) {
                    if (ignoreDeleted) 
                        cursor.currentPageOrderedEntries = cursor.currentPageOrderedEntries.filter(function(entry){ return !entry.__locally_deleted__ });
    ...
    //   - MERGE_ACCEPT_THEIRS     merge "theirs" and "yours" -- if the same field were changed locally and remotely, the remote value is kept
    ...
    Force.fetchSObjectsFromServer = function(config) {
        ...
        var soqlResponse = function(resp){
          var nextRecordsUrl = resp.nextRecordsUrl;
          return {
            totalSize: resp.totalSize,
            records: resp.records,
            hasMore: function() { return nextRecordsUrl != null; },
            getMore: function(batchSize) {
              var deferred = new $.Deferred(),
                  records=[],
                  recursiveMore = function(){
                    forcetkClient.queryMore(nextRecordsUrl, function(resp) {
                      nextRecordsUrl = resp.nextRecordsUrl;
                      records = records.concat(resp.records);
                      if(records.length >= batchSize || nextRecordsUrl == null){
                        records.totalSize = resp.totalSize;
        ...
        if (cache != null && cacheMode == Force.CACHE_MODE.CACHE_ONLY) {
            ...
            if (cache != null) {
                ...
                var processResult = function(resp) {
                    fetchResult = resp;
                    resp.records.totalSize = resp.totalSize;
                    return resp.records;
                };
    ...
    if (!_.isUndefined(Backbone)) {
        ...
        // To define the set of remote object to fetch, provide a implementation for the method fetchRemoteObjectsFromServer
        // Where the config is
        // config: {type:"cache", cacheQuery:<cache query>[, closeCursorImmediate:<true|false(default)>]} or something else understood by the fetchRemoteObjectsFromServer method of your subclass
