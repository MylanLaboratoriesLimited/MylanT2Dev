(function($, _, Backbone, forcetk){
  ...
  var promiser = function(object, methodName, objectName){
    var retfn = function(){
      ...
      var d = $.Deferred();
      if(objectName == "smartstoreClient" && typeof(args[0]) !== "boolean"){
        args.unshift(true);
      }
      args.push(function(){
  ...
  Force.init = function(creds, apiVersion, innerForcetkClient, reauth){
    ...
    Force.userId = creds.data ? creds.data.userId : creds.userId;
    ...
    forcetkClient.rest = promiser(innerForcetkClient, "rest", "forcetkClient");
    ...
    if(navigator.smartstore){
      ...
      smartstoreClient.impl = navigator.smartstore;
      smartstoreClient.registerSoup = promiser(navigator.smartstore, "registerSoup", "smartstoreClient");
      smartstoreClient.upsertSoupEntries = promiser(navigator.smartstore, "upsertSoupEntries", "smartstoreClient");
      ...
      smartstoreClient.clearSoup = promiser(navigator.smartstore, "clearSoup", "smartstoreClient");
  ...
  Force.Error = function(rawError){
    ...
    if(_.has(rawError, "responseText")){
      ...
      try{
        ...
        this.details = _.isArray(this.details) ? _.first(this.details) : this.details
    ...
    else if(_.has(rawError, "statusText")){
      this.type = "RestError";
      this.xhr = rawError;
      this.status = rawError.status;
      this.details = {}
      this.details.message = rawError.statusText;
      this.details.errorCode = rawError.status;
    }
    // Conflict error
  ...
  Force.StoreCache = function(soupName, additionalIndexSpecs, keyField, isGlobalStore){
    ...
    this.noMerge = true;
    this.isGlobalStore = isGlobalStore || true;
  };
  ...
  _.extend(Force.StoreCache.prototype, {
    ...
    save: function(record, noMerge, isLocal){
      if(this.soupName == null) return;
      ...
      return mergeIfRequested()
        .then(function(record){
          ...
          if(isLocal) return smartstoreClient.upsertSoupEntriesWithExternalId(that.isGlobalStore, that.soupName, [record], that.keyField)
          else return smartstoreClient.upsertSoupEntries(that.isGlobalStore, that.soupName, [record])
        })
    ...
    saveAll: function(records, noMerge){
      var that = this,
        batchSize = 1000,
        saved = [];
      if(noMerge == null) noMerge = that.noMerge

      var saveAllWrapper = function(records){
        if(that.soupName == null) return;
        ...
        var mergeIfRequested = function(){
          if(noMerge){
            ...
            if(_.any(records, function(record){
                return !_.has(record, that.keyField);
              })){
            ...
            var smartSql = "SELECT {" + that.soupName + ":_soup} "
              + "FROM {" + that.soupName + "} "
              + "WHERE {" + that.soupName + ":__local__} = 1 "
              + "AND {" + that.soupName + ":" + that.keyField + "} "
            ...
            return smartstoreClient.runSmartQuery(that.isGlobalStore, querySpec)
              ...
              .then(function(){
                var recordWithoutCompoundProperties = function(record){
                  var keys = _.keys(record);
                  var propertiesToIgnoreOnClient = _.filter(keys, function(key){
                    return _(key).contains('.')
                  });
                  return _.omit(record, propertiesToIgnoreOnClient);
                };
                return _.map(records, function(record){
                  var oldRecord = oldRecords[record[that.keyField]];
                  if(oldRecord) oldRecord = recordWithoutCompoundProperties(oldRecord);
                  if(that.mergeMode && that.mergeMode === Force.MERGE_MODE.MERGE_ACCEPT_YOURS){
                    return _.extend(record, oldRecord || {})
                  }
                  else if(that.mergeMode && that.mergeMode === Force.MERGE_MODE.MERGE_ACCEPT_THEIRS){
                    return _.extend(oldRecord || {}, record)
                  }
                  else{
                    return _.extend(record, oldRecord || {})
      ...
      var recursiveSaveAll = function(){
        return saveAllWrapper(records.splice(0, batchSize)).then(function(savedBatch){
          saved.push(savedBatch);
          if(records.length > 0){
            return recursiveSaveAll();
          }else{
    ...
    find: function(querySpec, ignoreDeleted){
      var cache = this;
      if(ignoreDeleted == null) ignoreDeleted = true
      ...
      var buildQueryResponse = function(cursor){
        var filterCursorEntries = function(cursor, ignoreDeleted){
          if(ignoreDeleted)
            cursor.currentPageOrderedEntries = cursor.currentPageOrderedEntries.filter(function(entry){
              return !entry.__locally_deleted__
            });
          return cursor.currentPageOrderedEntries;
        };

        return {
          records: filterCursorEntries(cursor, ignoreDeleted),
          hasMore: function(){
            return cursor != null &&
          ...
          getMore: function(){
            ...
            if(that.hasMore()){
              ...
              return smartstoreClient.moveCursorToNextPage(cache.isGlobalStore, cursor)
                ...
                .then(function(c){
                  cursor = c;
                  cursor.currentPageOrderedEntries = _.flatten(filterCursorEntries(cursor, ignoreDeleted));
    ...
    isLocalId: function(id){
      return id != null && (typeof(id) == 'number' || id.indexOf("local_") == 0);
    },
  ...
  Force.syncRemoteObjectWithCache = function(method, id, attributes, fieldlist, cache, localAction){
    ...
    var cacheUpdate = function(){
      ...
      return cache.save(data, true, isLocalId);
    };
  ...
  Force.syncSObjectWithServer = function(method, sobjectType, id, attributes, fieldlist){
    ...
    switch(method){
      case "create":
        promise = serverCreate();
        break;
      case "read":
        promise = serverRetrieve();
        break;
      case "update":
        promise = serverUpdate();
        break;
      case "delete":
        promise = serverDelete();
        break; /* XXX on 404 (record already deleted) we should not fail otherwise cache won't get cleaned up */
    }
  ...
  Force.syncRemoteObjectDetectConflict = function(method, id, attributes, fieldlist, cache, cacheMode, cacheForOriginals, mergeMode, syncWithServer){
      ...
      return cacheForOriginalsRetrieve()
        ...
        .then(function(remoteAttributes){
          ...
          if(remoteAttributes == null || originalAttributes == null){
            ...
            switch(mergeMode){
              case Force.MERGE_MODE.MERGE_ACCEPT_YOURS:
                shouldFail = false;
                break;
              case Force.MERGE_MODE.MERGE_FAIL_IF_CONFLICT:
                shouldFail = conflictingChanges.length > 0;
                break;
              case Force.MERGE_MODE.MERGE_FAIL_IF_CHANGED:
                shouldFail = remoteChanges.length > 0;
                break;
            }
            if(shouldFail){
              var conflictDetails = {
                base: originalAttributes,
                theirs: remoteAttributes,
                yours: attributes,
    ...
    switch(method){
      case "create":
        promise = syncRemoteObject(attributes).then(cacheForOriginalsSave);
        break;
      case "read":
        promise = syncRemoteObject(attributes).then(cacheForOriginalsSave);
        break;
      case "update":
        promise = checkConflictAndSync().then(cacheForOriginalsSave);
        break;
      case "delete":
        promise = checkConflictAndSync().then(cacheForOriginalsRemove);
        break;
    }
  ...
  // }
  //
  Force.fetchSObjectsFromServer = function(config, withoutCollecting){
          ...
          return {
            ...
            getMore: function(){
              ...
              return forcetkClient.queryMore(nextRecordsUrl).then(function(resp){
                ...
                that.records = withoutCollecting ? resp.records : that.records.concat(resp.records);
                return resp.records;
              });
            },
            closeCursor: function(){
    ...
    switch(config.type){
      case "soql":
        promise = serverSoql(config.query);
        break;
      case "sosl":
        promise = serverSosl(config.query);
        break;
      case "mru":
        promise = serverMru(config.sobjectType, config.fieldlist, config.orderBy, config.orderDirection);
        break;
        ...
        .then(function(resp){
          ...
          return {
            ...
            getMore: function(){
              ...
              return forcetkClient.queryMore(nextRecordsUrl).then(function(resp){
                nextRecordsUrl = resp.nextRecordsUrl;
                that.records = that.records.concat(resp.records);
                return resp.records;
              });
            },
            closeCursor: function(){
  ...
  if(!_.isUndefined(Backbone)){
    ...
    Force.RemoteObjectCollection = Backbone.Collection.extend({
      ...
      fetchRemoteObjectsFromCache: function(cache, cacheQuery, ignoreDeleted){
        return cache.find(cacheQuery, ignoreDeleted);
      },
      ...
      parse: function(resp, options){
        var that = this;
        return resp.map(function(result){
