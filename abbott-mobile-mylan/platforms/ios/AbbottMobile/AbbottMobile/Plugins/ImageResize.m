...
- (UIImage*) getImageUsingOptions:(NSDictionary*)options {
    ...
    if([imageDataType isEqualToString:@"base64Image"]==YES) {
        ...
        if ([imageData rangeOfString:@"file://"].location == NSNotFound) {
            imageData = [@"file://" stringByAppendingString:imageData];
        }
...
-(void)storeImage:(CDVInvokedUrlCommand*)command {
    ...
    BOOL photoAlbum = [[options objectForKey:@"photoAlbum"] boolValue] ?: NO;
    if(photoAlbum==YES) {
        ...
        NSString *savePath =  [options objectForKey:@"directory"] ?: documentsDirectory;
        if ([savePath rangeOfString:documentsDirectory].location == NSNotFound) {
            savePath = documentsDirectory;
        }
        NSMutableString* fullFileName = [NSMutableString stringWithString: savePath];
        NSFileManager *fileManager= [NSFileManager defaultManager];
        BOOL isDir = YES;
        if(![fileManager fileExistsAtPath:fullFileName isDirectory:&isDir])
            if(![fileManager createDirectoryAtPath:fullFileName withIntermediateDirectories:YES attributes:nil error:NULL])
                NSLog(@"Error: Create folder failed %@", fullFileName);
