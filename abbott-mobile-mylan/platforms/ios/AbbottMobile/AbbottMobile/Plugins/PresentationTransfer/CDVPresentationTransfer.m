...
#import "CDV.h"
#import "CDVPresentationTransfer.h"
#include <CFNetwork/CFNetwork.h>

@interface CDVPresentationTransfer ()
// Sets the requests headers for the request.
- (void)applyRequestHeaders:(NSDictionary*)headers toRequest:(NSMutableURLRequest*)req;
// Creates a delegate to handle an upload.
- (CDVPresentationTransferDelegate*)delegateForUploadCommand:(CDVInvokedUrlCommand*)command;
...
static const NSUInteger kStreamBufferSize = 64768;
// Magic value within the options dict used to set a cookie.
NSString* const kKeyCookie = @"__cookie";
// Form boundary for multi-part requests.
NSString* const kBoundary = @"+++++org.apache.cordova.formBoundary";
...
@implementation CDVPresentationTransfer
...
- (NSURLRequest*)requestForUploadCommand:(CDVInvokedUrlCommand*)command fileData:(NSData*)fileData
{
    ...
    CDVPresentationTransferError errorCode = 0;
    ...
    if (errorCode > 0) {
        result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsDictionary:[self createPresentationTransferError:errorCode AndSource:target AndTarget:server]];
    ...
    if ([options objectForKey:kKeyCookie]) {
        [req setValue:[options objectForKey:kKeyCookie] forHTTPHeaderField:@"Cookie"];
    ...
    for (NSString* key in options) {
        ...
        if (!val || (val == [NSNull null]) || [key isEqualToString:kKeyCookie]) {
            continue;
        }
    ...
    if (chunkedMode) {
        ...
        [self.commandDelegate runInBackground:^{
                if (CFWriteStreamOpen (writeStream)) {
                    ...
                    NSLog (@"PresentationTransfer: Failed to open writeStream");
                }
...
- (CDVPresentationTransferDelegate*)delegateForUploadCommand:(CDVInvokedUrlCommand*)command
{
    ...
    CDVPresentationTransferDelegate* delegate = [[CDVPresentationTransferDelegate alloc] init];
...
- (void)upload:(CDVInvokedUrlCommand*)command
{
    ...
    CDVPresentationTransferDelegate* delegate = [self delegateForUploadCommand:command];
...
{
    DLog(@"File Transfer abort downloading file...");
    NSString* objectId = [command.arguments objectAtIndex:0];

    CDVPresentationTransferDelegate* delegate = [activeTransfers objectForKey:objectId];
    ...
    if (delegate != nil) {
        ...
        CDVPluginResult* result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsDictionary:[self createPresentationTransferError:CONNECTION_ABORTED AndSource:delegate.source AndTarget:delegate.target]];
...
- (void)download:(CDVInvokedUrlCommand*)command
{
    ...
    BOOL trustAllHosts = [[command.arguments objectAtIndex:2 withDefault:[NSNumber numberWithBool:YES]] boolValue];
    ...
    if (errorCode > 0) {
        result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsDictionary:[self createPresentationTransferError:errorCode AndSource:sourceUrl AndTarget:filePath]];
        [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
        return;
    }
    ...
    NSString* parentPath = [delegate.target stringByDeletingLastPathComponent];
    ...
    delegate.fileHandle = [NSFileHandle fileHandleForWritingAtPath:delegate.target];
...
- (NSMutableDictionary*)createPresentationTransferError:(int)code AndSource:(NSString*)source AndTarget:(NSString*)target
{
    ...
    NSLog(@"PresentationTransferError %@", result);
...
- (void)dispose
{
    DLog(@"File Transfer reset");
    for (CDVPresentationTransferDelegate* delegate in [activeTransfers allValues]) {
        [delegate.connection cancel];
    }
...
@implementation CDVPresentationTransferDelegate

@synthesize callbackId, connection, source, target, responseData, fileHandle, command, bytesTransfered, bytesExpected, percentsLoaded, direction, responseCode, objectId, trustAllHosts;

- (void)connectionDidFinishLoading:(NSURLConnection*)connection
{
    NSString* uploadResponse = nil;
    BOOL downloadResponse = YES;
    ...
    if (self.direction == CDV_TRANSFER_UPLOAD) {
        if ((self.responseCode >= 200) && (self.responseCode < 300)) {
            ...
            result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsDictionary:[command createPresentationTransferError:CONNECTION_ERR AndSource:source AndTarget:target AndHttpStatus:self.responseCode]];
        }
    }
        ...
        if ((self.responseCode >= 200) && (self.responseCode < 300)) {
            @try {
                ...
                if (downloadResponse == NO) {
                    ...
                    result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsDictionary:[command createPresentationTransferError:INVALID_URL_ERR AndSource:source AndTarget:target AndHttpStatus:self.responseCode]];
                } else {
            ...
            @catch(id exception) {
                ...
                result = [CDVPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsDictionary:[command createPresentationTransferError:FILE_NOT_FOUND_ERR AndSource:source AndTarget:target AndHttpStatus:self.responseCode]];
            }
        } else {
            result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsDictionary:[command createPresentationTransferError:CONNECTION_ERR AndSource:source AndTarget:target AndHttpStatus:self.responseCode]];
        }
...
{
    CDVPluginResult* result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsDictionary:[command createPresentationTransferError:CONNECTION_ERR AndSource:source AndTarget:target AndHttpStatus:self.responseCode]];
...
- (void)updateStatus: (bool)lengthComputable
{
...
- (void)requestUpdateStatus:(NSUInteger)receivedChunkLength
{
    if (self.direction == CDV_TRANSFER_DOWNLOAD) {
        DLog(@"File Transfer live...");
        BOOL lengthComputable = (self.bytesExpected != NSURLResponseUnknownLength);
        if(lengthComputable){
            double newPercents = (double)(self.bytesTransfered + receivedChunkLength)/(double)self.bytesExpected*(double)100;
            if((newPercents-self.percentsLoaded)>=0.49 || self.bytesExpected<=(self.bytesTransfered + receivedChunkLength) || self.bytesTransfered == 0){
                [self updateStatus:lengthComputable];
                self.percentsLoaded = newPercents;
            }
        }else{
            [self updateStatus:lengthComputable];
        }
...
{
    if (self.direction == CDV_TRANSFER_DOWNLOAD) {
        [self requestUpdateStatus:data.length];
    }
